...
describes what users get. Pick a format with `-o`, or let hibp choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hibp <command> -o table   # aligned columns for reading
hibp <command> -o jsonl   # one JSON object per line, for piping
hibp <command> -o json    # a single JSON array
hibp <command> -o csv     # spreadsheet friendly
hibp <command> -o tsv     # tab-separated
hibp <command> -o url     # just the URL column
hibp <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hibp <command> --fields id,title,url
```
...
```bash
hibp <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hibp <command>            # a table, because this is a terminal
hibp <command> | wc -l    # JSONL, because this is a pipe
```
