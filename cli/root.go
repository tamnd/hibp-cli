// Package cli assembles the hibp command tree from the hibp
...
// NewApp assembles the kit application from the hibp domain. The
...
//
// To add a command, declare it in hibp/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := hibp.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(hibp.Domain{}).Register(app)
